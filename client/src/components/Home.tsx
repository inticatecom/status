 ...
 */
export default function Home({endpoints, server, status}: {
    ...
    const statusText = {
        title: status === "success" ? "All Systems Operational" : status === "warning" ? "Partial System Outage" : "All Systems Offline",
        summary: status === "success" ? "All services are currently determined to be online and accessible. To view the most up-to-date information, try refreshing the page to see if anything has changed." : status === "warning" ? "Some services are currently unavailable. Please read the categories below to identify which services have been affected." : "All services are currently experiencing an outage. Please check our socials to keep up-to-date with the issues."
    }
        ...
        <>
            <Callout className={"w-full mb-3"} type={status}>
                <div className={"flex justify-between items-center mb-1"}>
                    ...
                    <p className={"text-end"}>Updated {moment(new Date(server.latestPing)).fromNow()}</p>
                </div>
