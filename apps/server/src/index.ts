...
async function main(): Promise<void> {
  ...
  async function doCheck() {
    ...
    logger.debug(`Attempting to update ${String(total)} endpoint${total > 1 ? "s" : ""}.`);
    ...
    latestPing = new Date(); // Set the latest ping run to the current time after the ping operation has run.

    logger.info(`Successfully updated ${String(successful)}/${String(total)} endpoints.`);
  }
...
async function ping(site: Types.Endpoint, category: string): Promise<boolean> {
  const config = site.config ?? DEFAULT_CONFIG; // Find endpoint configuration or use default configuration.
  ...
  try {
    ...
    logger.warn(`Failed to resolve region for ${site.url}: ${String(e)}`);
  }
  ...
  states[category][siteName] = {
    online: result.ok || (config.okMethods?.some(method => method === result.status) ?? false),
...
void main(); // Initialize application.
