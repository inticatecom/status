...
export async function getLocalNetwork(): Promise<string | null> {
  try {
    const data = await (await ky.get<{ ip?: string }>(`https://api.ipify.org?format=json`)).json();
 ...
 */
export async function getRegion(ip: string): Promise<Types.NetworkRegion | null> {
  try {
    const data = await (
      await ky.get<Types.NetworkRegion | object>(
        `https://api.findip.net/${ip}/?token=${String(FIND_IP_KEY)}`
      )
    ).json();
