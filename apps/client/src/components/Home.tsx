  ...
  return (
    <>
      <button onClick={handleExpand} className="w-full mb-3 cursor-pointer">
        <Callout className={"w-full flex flex-col justify-center gap-1"} type={status}>
          ...
          {expanded && <p className={"text-sm text-start"}>{statusText.summary}</p>}
        </Callout>
      </button>
      <Status>
        {endpoints &&
          Object.entries(endpoints).map(([categoryName, categoryEndpoints], categoryIndex) => (
            <StatusCategory
              key={categoryIndex}
              title={categoryName}
              position={
